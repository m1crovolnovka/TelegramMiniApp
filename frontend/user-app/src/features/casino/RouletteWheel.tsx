...
export function RouletteWheel({ spinning, resultValue, onSpinEnd }: Props) {
  ...
  useEffect(() => {
    if (spinning && !prevSpinning.current && resultValue !== undefined) {
      ...
      const settleRotation = -(safeIndex * segmentAngle + segmentAngle / 2);
      setRotation((r) => r + 6 * 360 + settleRotation);
    }
  ...
  return (
    <div className="relative mx-auto flex h-72 w-72 items-center justify-center">
      <div className="absolute -top-3 z-10 text-3xl text-amber-400 drop-shadow">Ў</div>
      <div
        className="relative h-full w-full rounded-full border-4 border-amber-500 transition-transform duration-[4000ms] ease-out"
        style={{ transform: `rotate(${rotation}deg)`, background: conicGradient }}
      >
        {ROULETTE_WHEEL_ORDER.map((num, i) => {
          ...
          return (
            <span
              ...
              style={{ transform: `rotate(${angle}deg) translateY(-108px) rotate(${-angle}deg)`, transformOrigin: '0 0' }}
            >
              {num}
            </span>
          );
